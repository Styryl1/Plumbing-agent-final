...
const toOptionalString = (value?: string): string => {
...
export default function JobDrawer({
	...
	const availableSecondaryEmployees = useMemo(
		() => employees.filter((emp) => emp.id !== formData.primaryEmployeeId),
		[employees, formData.primaryEmployeeId],
	);
	type ConflictState = {
		mode: "update" | "reschedule" | "assignees";
	...
	const handleConflictConfirm = async (): Promise<void> => {
		...
		try {
			switch (conflictState.mode) {
				case "update":
					await updateJobMutation.mutateAsync(
						payloadWithOverride as Parameters<
							typeof updateJobMutation.mutateAsync
						>[0],
					);
					break;
				case "reschedule":
					await rescheduleMutation.mutateAsync(
						payloadWithOverride as Parameters<
							typeof rescheduleMutation.mutateAsync
						>[0],
					);
					break;
				case "assignees":
					await updateAssigneesMutation.mutateAsync(
						payloadWithOverride as Parameters<
							typeof updateAssigneesMutation.mutateAsync
						>[0],
					);
					break;
			}
		} catch (error) {
			console.error("Failed to override scheduling conflict:", error);
		}
	};
	...
	const handleSave = async (): Promise<void> => {
		...
		try {
			const rawCustomerId = formData.customerId?.trim();
			const normalizedCustomerId =
				rawCustomerId && rawCustomerId.length > 0 ? rawCustomerId : undefined;
			const currentCustomerId = job.customerId;
			const customerChanged =
				normalizedCustomerId !== undefined &&
				normalizedCustomerId !== currentCustomerId;
			...
			if (formData.title !== job.title) {
				patch.title = formData.title;
			}
			const nextDescription = toOptionalString(formData.description);
			const currentDescription = toOptionalString(job.description);
			...
			if (customerChanged) {
				patch.customerId = normalizedCustomerId;
			}
			...
			const normalizedSecondary = Array.from(
				new Set(
					formData.secondaryEmployeeIds.filter(
						(id) => id !== formData.primaryEmployeeId,
					),
				),
			);
			...
			if (
				...
				JSON.stringify([...normalizedSecondary].sort()) !==
										...
										onClick={() => {
											if (job) {
												...
												setFormData({
													...
													customerId: toOptionalString(job.customerId),
												});
											}
											setIsEditing(true);
										}}
								...
								{isEditing ? (
									<>
											...
											<Select
												...
												onValueChange={(value) => {
													setFormData((prev) => ({
														...
														secondaryEmployeeIds:
															prev.secondaryEmployeeIds.filter(
																(id) => id !== value,
															),
													}));
												}}
										...
										<div className="space-y-2">
											...
											{availableSecondaryEmployees.length === 0 ? (
												<p className="text-xs text-muted-foreground">
													{t("JobDrawer.secondaryEmployeesEmpty")}
												</p>
											) : (
												<div className="mt-1 max-h-40 space-y-2 overflow-y-auto pr-1">
													{availableSecondaryEmployees.map((emp) => {
														const checked =
															formData.secondaryEmployeeIds.includes(emp.id);
														return (
															<label
																key={emp.id}
																...
																<Checkbox
																	...
																	onCheckedChange={(state) => {
																		setFormData((prev) => {
																			...
																			return {
																				...prev,
																				secondaryEmployeeIds: Array.from(next),
																			};
																		});
																	}}
																	className="h-4 w-4"
											...
											<p className="text-xs text-muted-foreground">
												{t("JobDrawer.secondaryEmployeesHint")}
											</p>
										</div>
									</>
								) : (
